...
def getCalendarDates(date):
    ...
    next_month = date.replace(
        month=date.month % 12 + 1, year=date.year + (1 if date.month == 12 else 0))
    next_month_days = {datetime(next_month.year, next_month.month, day): "inactive" for day in range(
        1, 8 - ((len(prev_month_days) + len(month_days)) % 7))}
...
def home():
    ...
    documents = list(db['tasks'].find(
        {'username': username}, {'_id': 0}).limit(10))
...
def edit_for_specific_date(date):
    ...
    if request.method == 'POST':
        ...
        # update collection
        db['tasks'].update_one({'taskId': taskId}, {
...
def edit():    # POST handler
    ...
    if request.method == 'POST':
        ...
        # update collection
        db['tasks'].update_one({'taskId': taskId, 'username': flask_login.current_user.id}, {
                               '$set': {'task': task, 'date': date}})
...
def search():
    ...
    results = []
    if query:
        task_results = list(db.tasks.find(
            {"task": {"$regex": query, "$options": "i"}, "username": username}))
        data_results = list(db.tasks.find(
            {"date": {"$regex": query, "$options": "i"}, "username": username}))
...
def add_for_specific_date(date):
    ...
    if request.method == 'POST':
        ...
        # update collection
        db['tasks'].insert_one(
            {'taskId': taskId, 'task': task, 'date': date, 'username': username})
...
def delete():
    ...
    if request.method == 'POST':
        ...
        if taskId is not None:
            db['tasks'].delete_one(
                {'taskId': taskId, 'username': flask_login.current_user.id})
